...
import requestKegiatan, {
  KegiatanRequest,
  KegiatanResponseData,
} from "../features/request/kegiatanAPI";
...
import {
  paketRuasConfig,
  paketTitikConfig,
} from "../utils/map_config/paket_pekerjaan";
...
const LayerRenderer: React.FC<RendererProps> = (props: RendererProps) => {
    ...
    props.view
      ?.when(() => {
        ...
        if (!_.isEmpty(requestBody.kegiatan)) {
          ...
          mapServer.forEach((kegiatan) => {
            items.push(
              renderWMS[kegiatan](features.uptd, features.sup, features.tanggal)
            );
          });
  ...
  return (
    <Layers map={props.map} view={props.view} data={data} features={features} />
  );
};
...
const renderAPI: {
  [k: string]: (data: KegiatanResponseData) => __esri.LayerProperties;
} = {
  ...
  [KEGIATAN.VEHICLE_COUNTING]: (data) =>
    renderVehicleCounting(data.vehiclecounting),
  [KEGIATAN.LAPORAN_MASYARAKAT]: (data) =>
    renderLaporanMasyarakat(data.laporanmasyarakat),
  [KEGIATAN.LAPORAN_BENCANA]: (data) =>
    renderLaporanBencana(data.laporanbencana, data.iconlaporanbencana),
  [KEGIATAN.TITIK_RAWAN_BENCANA]: (data) =>
    renderRawanBencana(data.rawanbencana, data.iconrawanbencana),
  [KEGIATAN.SAPU_LOBANG]: (data) =>
    renderSapuLobang(data.sapulobang, data.iconsapulobang),
  [KEGIATAN.SAPU_LOBANG_PERENCANAAN]: (data) =>
    renderSapuLobangPerencanaan(data.sapulobang_perencanaan),
  [KEGIATAN.SAPU_LOBANG_PENANGANAN]: (data) =>
    renderSapuLobangPenanganan(data.sapulobang_penanganan),
  [KEGIATAN.LAPOR_RUMIJA]: (data) => renderLaporRumija(data.laporrumija),
  [KEGIATAN.PAKET_PEKERJAAN]: (data) => renderPaketPekerjaan(data.pembangunan),
};
